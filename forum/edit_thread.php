...
if(!empty($_POST['thread_edit'])) {
	if(isset($_GET['id'])){
		...
		$sql = "UPDATE threads SET thread_desc = ? WHERE thread_id = ?";
		$stmt = mysqli_stmt_init($conn);
		if (!mysqli_stmt_prepare($stmt, $sql)){
			echo $message = "Failed to edi thread.<br />";
		} else {
			mysqli_stmt_bind_param($stmt, "sd", $thread_desc, $thread_id);
...
<form method="post" action="<?php $_PHP_SELF ?>">
        Message:<br />
        <textarea name="thread_desc" rows = "10" /><?php echo strip_tags(nl2br(stripcslashes($thread_desc))) ?></textarea><br /><br />
